...
RUN apt-get update && \
  ...
  ./configure --with-http_ssl_module --with-http_gzip_static_module --with-http_realip_module --sbin-path=/usr/local/sbin && \
  make && \
  make install && \
  cd .. && \
